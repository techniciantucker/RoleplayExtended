...
function PlayerLoaded()
    for i=1, #AccessPoints, 1 do
        local atm = AccessPoints[i]
        ...
        RPX.Markers.Add("atm_" .. i, atm, "Tryck ~INPUT_CONTEXT~ för att öppna bankomaten", function()
            ShowBanking()
        end,
            {
                ["markerSize"] = {3.0, 3.0},
                ["markerColor"] = {255, 255, 255, 0}
            }
        )
    end
end
...
RegisterNetEvent("ATM:Close")
AddEventHandler("ATM:Close", function()
    SetNuiFocus(false, false)
end)

RegisterNetEvent("ATM:Withdrawal")
AddEventHandler("ATM:Withdrawal", function(amount)
    amount = tonumber(amount)

    local character = RPX.Character.GetCharacter()

    if character["bank"] >= amount then
        RPX.Character.UpdateData("cash",  character["cash"] + amount)
        RPX.Character.UpdateData("bank", character["bank"] - amount)
    else
        RPX.ShowNotification("Du har inte ~g~SEK " .. amount .. " ~w~på ditt bankkonto.")
    end
end)
...
AddEventHandler("ATM:Deposit", function(amount)
    ...
    if character["cash"] >= amount then
        ...
        RPX.ShowNotification("Du har inte ~g~SEK " .. amount .. " ~w~på dig.")
    end
end)